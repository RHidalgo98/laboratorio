...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($periodoZafra) || empty($fechaIngreso) || empty($num) || empty($hora) || empty($tacho) || empty($volFt3) || empty($brix) || empty($pol)) {
        ...
        header("Location: " . BASE_PATH . "forms/registrarMasaCocidaB.php?error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
        exit();
    }
    ...
    try {
        ...
        $checkQuery = "SELECT COUNT(*) FROM masacocidab WHERE (hora = :hora OR num = :num) AND periodoZafra = :periodoZafra AND fechaIngreso = :fechaIngreso";
        ...
        if ($stmt->execute()) {
            ...
            registrarBitacora(
            $_SESSION['nombre'],
            "Inserción de registro en Masa Cocida B", 
            $idRegistro,
            "masacocidab", 
            $detallesCambio
            );
            // Redirigir de vuelta a mostrar registros con filtros y mensaje de éxito
            header("Location: " . BASE_PATH . "controllers/masaCocidaB/mostrarMasaCocidaB.php?mensaje=Registro+insertado+correctamente&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
            exit();
        } else {
