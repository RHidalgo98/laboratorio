...
if (isset($_GET['id'])) {
    ...
    try {
        $stmt = $conexion->prepare("SELECT * FROM masacocidab WHERE idMasaCocidaB = :idMasaCocidaB");
        $stmt->bindParam(':idMasaCocidaB', $idMasaCocidaB, PDO::PARAM_INT);
        $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($error)) {
        try {
            ...
            $check_stmt->bindParam(':idMasaCocidaB', $idMasaCocidaB);
            $check_stmt->execute();
    ...
    if (empty($periodoZafra) || empty($fechaIngreso) || empty($num) || empty($hora) || empty($tacho) || empty($volFt3) || empty($brix) || empty($pol)) {
        ...
        header("Location: " . BASE_PATH . "controllers/masaCocidaB/editarMasaCocidaB.php?id=" . urlencode($idMasaCocidaB) . "&error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
        exit();
    }
    ...
    if (empty($error)) {
        try {
            $sql = "UPDATE masacocidab 
                    ...
                    WHERE idMasaCocidaB = :idMasaCocidaB";
            ...
            $stmt->bindParam(':idMasaCocidaB', $idMasaCocidaB);
            ...
            if (!empty($detallesCambio)) {
                registrarBitacora(
                    $_SESSION['nombre'],
                    "Edición de registro en Masa Cocida B",
                    $idMasaCocidaB,
                    "masacocidab",
                    $detallesCambio
                );
            ...
            $redirectUrl = BASE_PATH . "controllers/masaCocidaB/mostrarMasaCocidaB.php?mensaje=Registro+actualizado+correctamente";
        ...
        header("Location: " . BASE_PATH . "controllers/masaCocidaB/editarMasaCocidaB.php?id=" . urlencode($idMasaCocidaB) . "&error=" . urlencode($error));
        exit();
    }
}
?>
...
<head>
    ...
    <title>Editar Masa Cocida B</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-2 text-gray-800">Editar Registro de Masa Cocida B</h1>
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-body">
                            <form action="" method="POST">
                                ...
                                <a href="<?= BASE_PATH ?>controllers/masaCocidaB/mostrarMasaCocidaB.php?periodoZafra=<?= urlencode($periodoZafra); ?>&fechaIngreso=<?= urlencode($fechaIngreso); ?>" class="btn btn-secondary">Regresar</a>
                            </form>
                        </div>
                    </div>
                </div>
