...
if (isset($_GET['id']) && isset($_GET['periodoZafra']) && isset($_GET['fechaIngreso'])) {
    ...
    try {
        ...
        $stmtSelect = $conexion->prepare("SELECT * FROM mielfinal WHERE idMielFinal = :id");
        $stmtSelect->bindParam(':id', $idMielFinal, PDO::PARAM_INT);
        $stmtSelect->execute();
        ...
        if ($registro) {
            ...
            $detalles .= "Azúcar Reducido: {$registro['azucRed']}, ";
            $detalles .= "Observación: {$registro['observacion']}";

            // Eliminar el registro
            $stmtDelete = $conexion->prepare("DELETE FROM mielfinal WHERE idMielFinal = :id");
            $stmtDelete->bindParam(':id', $idMielFinal, PDO::PARAM_INT);

            if ($stmtDelete->execute()) {
                // Registrar en bitácora
                registrarBitacora(
                    $_SESSION['nombre'], // Asegurar que existe en la sesión
                    "Eliminación de registro en Miel Final",
                    $idMielFinal,
                    "mielfinal",
                    $detalles
                );
                ...
                $redirectUrl = BASE_PATH . "controllers/mielFinal/mostrarMielFinal.php?mensaje=Registro+eliminado+correctamente";
...
// Manejo de errores
if (!empty($error)) {
    $redirectUrl = BASE_PATH . "controllers/mielFinal/mostrarMielFinal.php?error=" . urlencode($error);
    $redirectUrl .= "&periodoZafra=" . urlencode($periodoZafra);
    $redirectUrl .= "&fechaIngreso=" . urlencode($fechaIngreso);
    header("Location: $redirectUrl");
    exit();
