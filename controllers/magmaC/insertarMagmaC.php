...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($periodoZafra) || empty($fechaIngreso) || empty($brix) || empty($pol)) {
        ...
        header("Location: " . BASE_PATH . "forms/registrarMagmaC.php?error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
        exit();
    }
    ...
    try {
        ...
        $checkQuery = "SELECT COUNT(*) FROM magmac WHERE hora = :hora AND periodoZafra = :periodoZafra AND fechaIngreso = :fechaIngreso";
        ...
        if ($checkStmt->fetchColumn() > 0) {
            ...
            header("Location: " . BASE_PATH . "forms/registrarMagmaC.php?error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso) . "&hora=" . urlencode($hora) . "&brix=" . urlencode($brix) . "&pol=" . urlencode($pol) . "&observacion=" . urlencode($observacion));
            exit();
        }

        // Consulta de inserción
        $sql = "INSERT INTO magmac (hora, brix, pol, observacion, periodoZafra, fechaIngreso) 
        ...
        if ($stmt->execute()) {
            ...
            registrarBitacora(
            $_SESSION['nombre'],
            "Inserción de registro en Magma C", 
            $idRegistro,
            "magmac", 
            $detallesCambio
            );
            // Redirigir de vuelta a mostrar registros con filtros y mensaje de éxito
            header("Location: " . BASE_PATH . "controllers/magmaC/mostrarMagmaC.php?mensaje=Registro+insertado+correctamente&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
            exit();
        } else {
