...
if (isset($_GET['id'])) {
    ...
    try {
        $stmt = $conexion->prepare("SELECT * FROM magmac WHERE idMagmaC = :idMagmaC");
        $stmt->bindParam(':idMagmaC', $idMagmaC, PDO::PARAM_INT);
        $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($error)) {
        try {
            ...
            $check_stmt->bindParam(':idMagmaC', $idMagmaC);
            $check_stmt->execute();
    ...
    if (empty($periodoZafra) || empty($fechaIngreso) || empty($brix) || empty($pol)) {
        ...
        header("Location: " . BASE_PATH . "controllers/magmaC/editarMagmaC.php?id=" . urlencode($idMagmaC) . "&error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
        exit();
    }
    ...
    if (empty($error)) {
        try {
            ...
            $stmt->bindParam(':idMagmaC', $idMagmaC);
            ...
            registrarBitacora(
                $_SESSION['nombre'],
                "Edición de registro en Magma C",
                $idMagmaC,
                "magmac",
                $detallesCambio
            );
            ...
            $redirectUrl = BASE_PATH . "controllers/magmaC/mostrarMagmaC.php?mensaje=Registro+actualizado+correctamente";
        ...
        header("Location: " . BASE_PATH . "controllers/magmaC/editarMagmaC.php?id=" . urlencode($idMagmaC) . "&error=" . urlencode($error));
        exit();
    }
}
?>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-2 text-gray-800">Editar Registro de Magma C</h1>
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-body">
                            <form action="" method="POST">
                                ...
                                <a href="<?= BASE_PATH ?>controllers/magmaC/mostrarMagmaC.php?periodoZafra=<?= !empty($periodoZafra) ? urlencode($periodoZafra) : ''; ?>&fechaIngreso=<?= !empty($fechaIngreso) ? urlencode($fechaIngreso) : ''; ?>" class="btn btn-secondary">Regresar</a>
                            </form>
                        </div>
                    </div>
                </div>
