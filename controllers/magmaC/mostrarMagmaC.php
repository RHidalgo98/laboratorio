        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <h1 class="h3 mb-2 text-gray-800">Registros de Magma C</h1>
                    ...
                    <?php if (!empty($periodoZafra) && !empty($fechaIngreso)): ?>
                        <div class="mb-4">
                            <a href="<?= BASE_PATH ?>forms/registrarMagmaC.php?periodoZafra=<?= urlencode($periodoZafra); ?>&fechaIngreso=<?= urlencode($fechaIngreso); ?>" class="btn btn-primary">
                    ...
                    if (!empty($periodoZafra) && !empty($fechaIngreso)) {
                        ...
                        $query = "SELECT idMagmaC, hora, brix, pol, observacion, periodoZafra, fechaIngreso
                                  FROM magmac
                                  WHERE periodoZafra = :periodoZafra AND fechaIngreso = :fechaIngreso
                                  ORDER BY idMagmaC ASC";
                        ...
                        if ($result) {
                            ...
                            echo "<table class='table table-bordered' id='tablaMagmaC' width='100%' cellspacing='0'>";
                            ...
                            foreach ($result as $row) {
                                ...
                                $brix = $row['brix'];
                                $pol = $row['pol'];
                                $idMagmaC = $row['idMagmaC'];
                                $observacion = !empty($row['observacion']) ? $row['observacion'] : '-';

                                // Sumar brix y pol para el cálculo total
                                $totalBrix += $brix;
                                $totalPol += $pol;

                                // Calcular pureza magma b
                                $purMagma = ($brix != 0) ? round(($pol * 100) / $brix, 2) : 0;
                                ...
                                echo "<tr>
                                        ...
                                        <td>
                                            <a href='" . BASE_PATH . "controllers/magmaC/editarMagmaC.php?id=" . urlencode($idMagmaC) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso) . "' class='text-warning me-2 action-icons'>
                                                <i class='fas fa-edit'></i>
                                            </a>
                                            <a href='" . BASE_PATH . "controllers/magmaC/eliminarMagmaC.php?id=" . urlencode($idMagmaC) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso) . "' 
