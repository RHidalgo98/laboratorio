...
if (isset($_GET['id']) && isset($_GET['periodoZafra']) && isset($_GET['fechaIngreso'])) {
    ...
    try {
        ...
        $stmtSelect = $conexion->prepare("SELECT * FROM meladura WHERE idMeladura = :id");
        $stmtSelect->bindParam(':id', $idMeladura, PDO::PARAM_INT);
        $stmtSelect->execute();
        ...
        if ($registro) {
            ...
            $stmtDelete = $conexion->prepare("DELETE FROM meladura WHERE idMeladura = :id");
            $stmtDelete->bindParam(':id', $idMeladura, PDO::PARAM_INT);

            if ($stmtDelete->execute()) {
                // Registrar en bitácora
                registrarBitacora(
                    $_SESSION['nombre'], // Asegurar que existe en la sesión
                    "Eliminación de registro en Meladura",
                    $idMeladura,
                    "meladura",
                    $detalles
                );
                ...
                $redirectUrl = BASE_PATH . "controllers/meladura/mostrarMeladura.php?mensaje=Registro+eliminado+correctamente";
...
// Manejo de errores
if (!empty($error)) {
    $redirectUrl = BASE_PATH . "controllers/meladura/mostrarMeladura.php?error=" . urlencode($error);
    $redirectUrl .= "&periodoZafra=" . urlencode($periodoZafra);
    $redirectUrl .= "&fechaIngreso=" . urlencode($fechaIngreso);
    header("Location: $redirectUrl");
    exit();
