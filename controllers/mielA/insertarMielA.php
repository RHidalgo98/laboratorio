...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($periodoZafra) || empty($fechaIngreso) || empty($hora) || empty($num) || empty($brix) || empty($pol)) {
        ...
        header("Location: " . BASE_PATH . "forms/registrarMielA.php?id=" . urlencode($idMielA) . "&error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
        exit();
    }
    ...
    if ($brix <= 0 || $pol <= 0) {
        ...
        header("Location: " . BASE_PATH . "forms/registrarMielA.php?error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
        exit();
    }

    try {
        // Verificar duplicados
        $checkQuery = "SELECT COUNT(*) FROM miela WHERE (hora = :hora OR num = :num) AND periodoZafra = :periodoZafra AND fechaIngreso = :fechaIngreso";
        ...
        if ($checkStmt->fetchColumn() > 0) {
            ...
            header("Location: " . BASE_PATH . "forms/registrarMielA.php?error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso) . "&hora=" . urlencode($hora) . "&num=" . urlencode($num) . "&brix=" . urlencode($brix) . "&pol=" . urlencode($pol) . "&observacion=" . urlencode($observacion));
            exit();
        }

        // Verificar que el num existe en Masa Cocida A
        $query = "SELECT brix, pol FROM masacocidaa 
        ...
        if (!$masaData) {
            throw new Exception('No se encontró el num en Masa Cocida A');
        }

        // Insertar registro en la base de datos
        $sql = "INSERT INTO miela (periodoZafra, fechaIngreso, hora, num, brix, pol, observacion) VALUES (:periodoZafra, :fechaIngreso, :hora, :num, :brix, :pol, :observacion)";
        ...
        if ($stmt->execute()) {
            ...
            registrarBitacora(
            $_SESSION['nombre'],
            "Inserción de registro en Miel A", 
            $idRegistro,
            "miela", 
            $detallesCambio
            );
            // Redirigir a mostrarMielA.php con un mensaje de éxito
            header("Location: " . BASE_PATH . "controllers/mielA/mostrarMielA.php?periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso) . "&mensaje=Registro+insertado+correctamente");
            exit();
        } else {
