...
if (isset($_GET['id'])) {
    ...
    try {
        $stmt = $conexion->prepare("SELECT * FROM miela WHERE idMielA = :idMielA");
        $stmt->bindParam(':idMielA', $idMielA, PDO::PARAM_INT);
        $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($error)) {
        try {
            $check_sql = "SELECT COUNT(*) FROM miela
            ...
            $check_stmt->bindParam(':idMielA', $idMielA);
            $check_stmt->execute();
    ...
    if (empty($periodoZafra) || empty($fechaIngreso) || empty($hora) || empty($num) || empty($brix) || empty($pol)) {
        ...
        header("Location: " . BASE_PATH . "controllers/mielA/editarMielA.php?id=" . urlencode($idMielA) . "&error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
        exit();
    }
    ...
    if (empty($error)) {
        try {
            ...
            $stmt->bindParam(':idMielA', $idMielA);
        ...
        header("Location: " . BASE_PATH . "controllers/mielA/editarMielA.php?id=" . urlencode($idMielA) . "&error=" . urlencode($error));
        exit();
    }
}
?>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-2 text-gray-800">Editar Registro de Miel A</h1>
