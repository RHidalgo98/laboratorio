...
if (isset($_GET['id'])) {
    ...
    try {
        $stmt = $conexion->prepare("SELECT * FROM jugomezcladophmbc WHERE idJugoMezcladoPHMBC = :idJugoMezcladoPHMBC");
        $stmt->bindParam(':idJugoMezcladoPHMBC', $idJugoMezcladoPHMBC, PDO::PARAM_INT);
        $stmt->execute();
...
$query = "SELECT COUNT(*) as count FROM jugomezcladophmbc WHERE periodoZafra = :periodoZafra AND fechaIngreso = :fechaIngreso";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($error)) {
        try {
            $check_sql = "SELECT COUNT(*) FROM jugomezcladophmbc 
                          WHERE hora = :hora 
                          AND periodoZafra = :periodoZafra 
                          AND fechaIngreso = :fechaIngreso 
                          AND idJugoMezcladoPHMBC != :idJugoMezcladoPHMBC";
            ...
            $check_stmt->bindParam(':idJugoMezcladoPHMBC', $idJugoMezcladoPHMBC);
            $check_stmt->execute();
    ...
    if (empty($error)) {
        try {
            $sql = "UPDATE jugomezcladophmbc 
                    ...
                    WHERE idJugoMezcladoPHMBC = :idJugoMezcladoPHMBC";

            $stmt = $conexion->prepare($sql);
            $stmt->bindParam(':hora', $hora);
            $stmt->bindValue(':marcador', $marcador);
            ...
            $stmt->bindParam(':idJugoMezcladoPHMBC', $idJugoMezcladoPHMBC);
        ...
        header("Location: " . BASE_PATH . "controllers/jugoMezcladoPHMBC/editarJugoMezcladoPHMBC.php?id=" . urlencode($idJugoMezcladoPHMBC) . "&error=" . urlencode($error));
        exit();
    }
}
?>
...
<head>
    ...
    <title>Editar Jugo Mezclado</title>
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-body">
                            <form action="" method="POST">
                                ...
                                <a href="<?= BASE_PATH ?>controllers/jugoMezcladoPHMBC/mostrarJugoMezcladoPHMBC.php?periodoZafra=<?= !empty($periodoZafra) ? urlencode($periodoZafra) : ''; ?>&fechaIngreso=<?= !empty($fechaIngreso) ? urlencode($fechaIngreso) : ''; ?>" class="btn btn-secondary mt-3">Regresar</a>
                            </form>
                        </div>
                    </div>
                </div>
