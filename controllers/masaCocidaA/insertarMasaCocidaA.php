...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Recibir las entradas del formulario
    $num = $_POST['num'];
    $hora = $_POST['hora'];
    $tacho = $_POST['tacho'];
    $volFt3 = $_POST['volFt3'];
    ...
    if (empty($periodoZafra) || empty($fechaIngreso) || empty($num) || empty($hora) || empty($tacho) || empty($volFt3) || empty($brix) || empty($pol)) {
        ...
        header("Location: " . BASE_PATH . "forms/registrarMasaCocidaA.php?error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
        exit();
    }
    ...
    try {
        ...
        $checkQuery = "SELECT COUNT(*) FROM masacocidaa WHERE (hora = :hora OR num = :num) AND periodoZafra = :periodoZafra AND fechaIngreso = :fechaIngreso";
        ...
        if ($stmt->execute()) {
            ...
            registrarBitacora(
            $_SESSION['nombre'],
            "Inserción de registro en Masa Cocida A", 
            $idRegistro,
            "masacocidaa", 
            $detallesCambio
            );
            // Redirigir de vuelta a mostrar registros con filtros y mensaje de éxito
            header("Location: " . BASE_PATH . "controllers/masaCocidaA/mostrarMasaCocidaA.php?mensaje=Registro+insertado+correctamente&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
            exit();
        } else {
