...
if (isset($_GET['id']) && isset($_GET['periodoZafra']) && isset($_GET['fechaIngreso'])) {
    ...
    try {
        ...
        $stmtSelect = $conexion->prepare("SELECT * FROM masacocidaa WHERE idMasaCocidaA = :id");
        $stmtSelect->bindParam(':id', $idMasaCocidaA, PDO::PARAM_INT);
        $stmtSelect->execute();
        ...
        if ($registro) {
            ...
            $stmtDelete = $conexion->prepare("DELETE FROM masacocidaa WHERE idMasaCocidaA = :id");
            $stmtDelete->bindParam(':id', $idMasaCocidaA, PDO::PARAM_INT);

            if ($stmtDelete->execute()) {
                // Registrar en bitácora
                registrarBitacora(
                    $_SESSION['nombre'], // Asegurar que existe en la sesión
                    "Eliminación de registro en Masa Cocida A",
                    $idMasaCocidaA,
                    "masacocidaa",
                    $detalles
                );
                ...
                $redirectUrl = BASE_PATH . "controllers/masaCocidaA/mostrarMasaCocidaA.php?mensaje=Registro+eliminado+correctamente";
...
// Manejo de errores
if (!empty($error)) {
    $redirectUrl = BASE_PATH . "controllers/masaCocidaA/mostrarMasaCocidaA.php?error=" . urlencode($error);
    $redirectUrl .= "&periodoZafra=" . urlencode($periodoZafra);
    $redirectUrl .= "&fechaIngreso=" . urlencode($fechaIngreso);
    header("Location: $redirectUrl");
    exit();
