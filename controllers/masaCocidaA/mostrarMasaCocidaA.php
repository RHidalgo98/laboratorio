...
<head>
    ...
    <title>Mostrar Masa Cocida A</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <h1 class="h3 mb-2 text-gray-800">Registros de Masa Cocida A</h1>
                    ...
                    <?php if (!empty($periodoZafra) && !empty($fechaIngreso)): ?>
                        <div class="mb-4">
                            <a href="<?= BASE_PATH ?>forms/registrarMasaCocidaA.php?periodoZafra=<?= urlencode($periodoZafra); ?>&fechaIngreso=<?= urlencode($fechaIngreso); ?>" class="btn btn-primary">
                    ...
                    if (!empty($periodoZafra) && !empty($fechaIngreso)) {
                        ...
                        $query = "SELECT ma.idMasaCocidaA, ma.num, ma.hora, ma.tacho, ma.volFt3, ma.brix, ma.pol, ma.periodoZafra, ma.fechaIngreso, 
                                         mia.brix AS brix_miel, mia.pol AS pol_miel, ma.observacion
                                  FROM masacocidaa ma
                                  LEFT JOIN miela mia ON ma.periodoZafra = mia.periodoZafra AND ma.fechaIngreso = mia.fechaIngreso AND ma.num = mia.num
                                  WHERE ma.periodoZafra = :periodoZafra AND ma.fechaIngreso = :fechaIngreso
                                  ORDER BY ma.num ASC";
                        ...
                        if (!empty($periodoZafra) && !empty($fechaIngreso)) {
                            try {
                                ...
                                $queryMasa = "SELECT SUM(pol) AS sumPolMasa, SUM(brix) AS sumBrixMasa FROM masacocidaa 
                                ...
                                $queryMiel = "SELECT SUM(pol) AS sumPolMiel, SUM(brix) AS sumBrixMiel, COUNT(*) AS countMiel FROM miela
                                          WHERE periodoZafra = :periodoZafra AND fechaIngreso = :fechaIngreso AND brix > 0";
                                ...
                                $promRendCristal = ($promPurMasa != 0 && $promPurMiel != 0 && (100 - $promPurMiel) != 0) ?
                                    round((($promPurMasa - $promPurMiel) / (100 - $promPurMiel)) * 100, 2) : 0;
                            } catch (PDOException $e) {
                        ...
                        if ($result) {
                            ...
                            echo "<table id='tablaMasaCocidaA' class='table table-bordered' width='100%' cellspacing='0'>";
                            ...
                            foreach ($result as $row) {
                                ...
                                $idMasaCocidaA = $row['idMasaCocidaA'];
                                ...
                                $rendCristal = ($purMasa != 0 && $purMiel != 0) ? round((($purMasa - $purMiel) / (100 - $purMiel)) * 100, 2) : 0;
                                ...
                                $purMasaValores[] = $purMasa;
                                $purMielValores[] = $purMiel;

                                $cristaliz = ($tacho == 8) ? 3 : (($tacho == 7) ? 5 : 4);
                                ...
                                echo "<tr>
                                        ...
                                        <td>
                                            <a href='" . BASE_PATH . "controllers/masaCocidaA/editarMasaCocidaA.php?id=" . urlencode($idMasaCocidaA) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso) . "' class='text-warning me-2 action-icons'>
                                                <i class='fas fa-edit'></i>
                                            </a>
                                            <a href='" . BASE_PATH . "controllers/masaCocidaA/eliminarMasaCocidaA.php?id=" . urlencode($idMasaCocidaA) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso) . "' 
                            ...
                            // Calcular promPurMasa utilizando promBrix y promPol
                            $promPurMasa = ($promBrix != 0) ? round(($promPol * 100) / $promBrix, 2) : 0;

                            // Calcular promRendCristal utilizando promPurMasa y promPurMiel
                            $promRendCristal = ($promPurMasa != 0 && $promPurMiel != 0) ? round((($promPurMasa - $promPurMiel) / (100 - $promPurMiel)) * 100, 2) : 0;
