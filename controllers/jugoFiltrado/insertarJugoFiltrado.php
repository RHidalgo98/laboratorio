...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($periodoZafra) || empty($fechaIngreso) || empty($hora) || empty($brix) || empty($sac)) {
        ...
        header("Location: " . BASE_PATH . "forms/registrarJugoFiltrado.php?id=" . urlencode($idFiltrado) . "&error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
        exit();
    }
    ...
    if (!empty($error)) {
        header("Location: " . BASE_PATH . "forms/registrarJugoFiltrado.php?error=" . urlencode($error)
    ...
    try {
        // Comprobar si ya existe un registro con la misma hora, periodo de zafra y fecha de ingreso
        ...
        if ($count > 0) {
            // Si ya existe un registro en la misma hora, mostrar el mensaje de error
            ...
            // SQL de inserción con periodoZafra y fechaIngreso
            $sql = "INSERT INTO jugofiltrado (hora, observacion, brix, sac, mlGastado, periodoZafra, fechaIngreso)
                    VALUES (:hora, :observacion, :brix, :sac, :mlGastado, :periodoZafra, :fechaIngreso)";
            ...
            if ($stmt->execute()) {
                ...
                $detallesCambio = "Hora: $hora, Observación: $observacion, Brix: $brix, SAC: $sac, ML Gastado: $mlGastado";

                // Añadir entrada a la bitácora
                registrarBitacora(
                    $_SESSION['nombre'],
                    "Inserción de registro en Jugo Filtrado", 
                    $idRegistro,
                    "jugofiltrado", 
                    $detallesCambio
                );
                // Redirigir a la página de mostrar registros con los filtros seleccionados
                header("Location: " . BASE_PATH . "controllers/jugoFiltrado/mostrarJugoFiltrado.php?mensaje=Registro+insertado+correctamente&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
                exit();
            } else {
    ...
    header("Location: " . BASE_PATH . "forms/registrarJugoFiltrado.php?error=" . urlencode("Solicitud no válida."));
    exit();
}
?>
