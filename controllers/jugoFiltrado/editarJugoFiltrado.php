...
if (isset($_GET['id'])) {
    ...
    try {
        $stmt = $conexion->prepare("SELECT * FROM jugofiltrado WHERE idJugoFiltrado = :idJugoFiltrado");
        $stmt->bindParam(':idJugoFiltrado', $idJugoFiltrado, PDO::PARAM_INT);
        $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $check_sql = "SELECT COUNT(*) FROM jugofiltrado 
                      WHERE hora = :hora 
                      AND periodoZafra = :periodoZafra 
                      AND fechaIngreso = :fechaIngreso 
                      AND idJugoFiltrado != :idJugoFiltrado";
        ...
        $check_stmt->bindParam(':idJugoFiltrado', $idJugoFiltrado);
        $check_stmt->execute();
        $count = $check_stmt->fetchColumn();

        if ($count > 0) {
            $error = "Ya existe un registro con la misma hora, periodo de zafra y fecha.";
            header("Location: " . BASE_PATH . "controllers/jugoFiltrado/editarJugoFiltrado.php?id=" . urlencode($idJugoFiltrado) . "&error=" . urlencode($error));
            exit();
        }
    } catch (PDOException $e) {
    ...
    if (empty($periodoZafra) || empty($fechaIngreso) || empty($hora) || empty($brix) || empty($sac)) {
        ...
        header("Location: " . BASE_PATH . "controllers/jugoFiltrado/editarJugoFiltrado.php?id=" . urlencode($idJugoFiltrado) . "&error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
        exit();
    }

    // Validar que Brix, SAC y ML Gastado no sean negativos
    if ($brix <= 0) {
    ...
    try {
        $sql = "UPDATE jugofiltrado 
                SET hora = :hora, 
                    observacion = :observacion, 
                    brix = :brix, 
                    sac = :sac, 
                    mlGastado = :mlGastado 
                WHERE idJugoFiltrado = :idJugoFiltrado";
        ...
        $stmt->bindParam(':idJugoFiltrado', $idJugoFiltrado);
        ...
        if ($stmt->execute()) {
            ...
            $url = BASE_PATH . "controllers/jugoFiltrado/mostrarJugoFiltrado.php?mensaje=Registro+actualizado+correctamente&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso);
            header("Location: $url");
            exit();
        } else {
...
<head>
    ...
    <title>Editar Registro de Jugo Filtrado</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-2 text-gray-800">Editar Registro de Jugo Filtrado</h1>
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-body">
                            <form action="<?php echo BASE_PATH; ?>controllers/jugoFiltrado/editarJugoFiltrado.php?id=<?php echo $idJugoFiltrado; ?>" method="POST">
                                <div class="mb-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <a href="<?= BASE_PATH ?>controllers/jugoFiltrado/mostrarJugoFiltrado.php?periodoZafra=<?= urlencode($periodoZafra); ?>&fechaIngreso=<?= urlencode($fechaIngreso); ?>" class="btn btn-secondary">Regresar</a>
                                </div>
                            </form>
                        </div>
                    </div>
