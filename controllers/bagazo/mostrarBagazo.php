...
if (!empty($periodoZafra) && !empty($fechaIngreso)) {
    $query = "SELECT DATE_FORMAT(hora, '%H:%i') AS hora, pol, humedad, observacion, idBagazo 
              FROM bagazo 
...
$sumas = ['pol' => 0, 'humedad' => 0];
$contadores = ['pol' => 0, 'humedad' => 0];
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-2 text-gray-800">Registros de Bagazo</h1>
                    ...
                    <?php if (!empty($periodoZafra) && !empty($fechaIngreso)): ?>
                        <div class="mb-4">
                            <a href="<?= BASE_PATH ?>forms/registrarBagazo.php?periodoZafra=<?= urlencode($periodoZafra); ?>&fechaIngreso=<?= urlencode($fechaIngreso); ?>" class="btn btn-primary">
                                            ...
                                            <tbody>
                                                <?php foreach ($datosPorHora as $registro): ?>
                                                    <tr>
                                                        ...
                                                        <td class="<?= $registro['pol'] !== null ? '' : 'empty' ?>"><?= formatear_valor($registro['pol']) ?></td>
                                                        <td class="<?= $registro['humedad'] !== null ? '' : 'empty' ?>"><?= formatear_valor($registro['humedad']) ?></td>
                                                        <td class="observacion"><?= htmlspecialchars(!empty($registro['observacion']) ? $registro['observacion'] : '-') ?></td>
                                                        <td>
                                                            <?php if ($registro['idBagazo'] !== null): ?>
                                                                <a href="<?= BASE_PATH ?>controllers/bagazo/editarBagazo.php?id=<?= urlencode($registro['idBagazo']) ?>" class="text-warning me-2">
                                                                    <i class="fas fa-edit"></i>
                                                                </a>
                                                                <a href="<?= BASE_PATH ?>controllers/bagazo/eliminarBagazo.php?id=<?= urlencode($registro['idBagazo']) ?>&periodoZafra=<?= urlencode($periodoZafra) ?>&fechaIngreso=<?= urlencode($fechaIngreso) ?>"
                                                ...
                                                <tr class="table-info">
                                                    ...
                                                    <td><?= formatear_valor($promedios['pol']) ?></td>
                                                    <td><?= formatear_valor($promedios['humedad']) ?></td>
                                                    <td></td>
                                                    <td></td>
                                                </tr>
                                            </tbody>
