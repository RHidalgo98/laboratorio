...
if (isset($_GET['id']) && isset($_GET['periodoZafra']) && isset($_GET['fechaIngreso'])) {
    ...
    try {
        ...
        $stmtSelect = $conexion->prepare("SELECT * FROM magmab WHERE idMagmaB = :id");
        $stmtSelect->bindParam(':id', $idMagmaB, PDO::PARAM_INT);
        $stmtSelect->execute();
        ...
        if ($registro) {
            ...
            $stmtDelete = $conexion->prepare("DELETE FROM magmab WHERE idMagmaB = :id");
            $stmtDelete->bindParam(':id', $idMagmaB, PDO::PARAM_INT);

            if ($stmtDelete->execute()) {
                // Registrar en bitácora
                registrarBitacora(
                    $_SESSION['nombre'], // Asegurar que existe en la sesión
                    "Eliminación de registro en Magma B",
                    $idMagmaB,
                    "magmab",
                    $detalles
                );
                ...
                $redirectUrl = BASE_PATH . "controllers/magmaB/mostrarMagmaB.php?mensaje=Registro+eliminado+correctamente";
...
// Manejo de errores
if (!empty($error)) {
    $redirectUrl = BASE_PATH . "controllers/magmaB/mostrarMagmaB.php?error=" . urlencode($error);
    $redirectUrl .= "&periodoZafra=" . urlencode($periodoZafra);
    $redirectUrl .= "&fechaIngreso=" . urlencode($fechaIngreso);
    header("Location: $redirectUrl");
    exit();
