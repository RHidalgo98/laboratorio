...
if (isset($_GET['id'])) {
    ...
    try {
        $stmt = $conexion->prepare("SELECT * FROM jugomezclado WHERE idJugoMezclado = :idJugoMezclado");
        $stmt->bindParam(':idJugoMezclado', $idJugoMezclado, PDO::PARAM_INT);
        $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $check_sql = "SELECT COUNT(*) FROM jugomezclado 
                      WHERE hora = :hora 
                      AND periodoZafra = :periodoZafra 
                      AND fechaIngreso = :fechaIngreso 
                      AND idJugoMezclado != :idJugoMezclado";
        ...
        $check_stmt->bindParam(':idJugoMezclado', $idJugoMezclado);
        $check_stmt->execute();
        $count = $check_stmt->fetchColumn();

        if ($count > 0) {
            $error = "Ya existe un registro con la misma hora, periodo de zafra y fecha.";
            header("Location: " . BASE_PATH . "controllers/jugoMezclado/editarJugoMezclado.php?id=" . urlencode($idJugoMezclado) . "&error=" . urlencode($error));
            exit();
        }
    } catch (PDOException $e) {
    ...
    if (empty($periodoZafra) || empty($fechaIngreso) || empty($hora) || empty($brix) || empty($sac)) {
        ...
        header("Location: " . BASE_PATH . "controllers/jugoMezclado/editarJugoMezclado.php?id=" . urlencode($idJugoMezclado) . "&error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
        exit();
    }

    // Validaciones
    if ($fosfato < 0) {
        $error = "El valor de Fosfato no puede ser negativo.";
    } elseif ($brix <= 0) {
        ...
        $pureza = ($brix != 0) ? round(($sac * 100) / $brix, 2) : 0;
    }
    ...
    try {
        $sql = "UPDATE jugomezclado 
                SET hora = :hora, 
                    ...
                    sac = :sac, 
                    mlGastado = :mlGastado 
                WHERE idJugoMezclado = :idJugoMezclado";

        $stmt = $conexion->prepare($sql);
        $stmt->bindParam(':hora', $hora);
        $stmt->bindParam(':observacion', $observacion);
        $stmt->bindParam(':fosfato', $fosfato);
        $stmt->bindParam(':brix', $brix);
        $stmt->bindParam(':sac', $sac);
        $stmt->bindParam(':mlGastado', $mlGastado);
        $stmt->bindParam(':idJugoMezclado', $idJugoMezclado);
        ...
        if ($stmt->execute()) {
            ...
            if ($registro['fosfato'] != $fosfato) {
                $detallesCambio[] = "Fosfato: {$registro['fosfato']} → $fosfato";
            }
            ...
            if (!empty($detallesCambio)) {
                registrarBitacora(
                    $_SESSION['nombre'],
                    "Actualización de registro en Jugo Mezclado",
                    $idJugoMezclado,
                    "jugomezclado",
            ...
            $url = BASE_PATH . "controllers/jugoMezclado/mostrarJugoMezclado.php?mensaje=Registro+actualizado+correctamente";
...
<head>
    ...
    <title>Editar Registro de Jugo Mezclado</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-2 text-gray-800">Editar Registro de Jugo Mezclado</h1>
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-body">
                            <form action="<?php echo BASE_PATH; ?>controllers/jugoMezclado/editarJugoMezclado.php?id=<?php echo $idJugoMezclado; ?>" method="POST">
                                <div class="mb-3">
                                ...
                                <div class="mb-3">
                                    <label for="fosfato" class="form-label">Fosfato:</label>
                                    <input type="number" id="fosfato" name="fosfato" class="form-control" step="0.01" min="0" value="<?php echo $registro['fosfato']; ?>">
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <a href="<?= BASE_PATH ?>controllers/jugoMezclado/mostrarJugoMezclado.php?periodoZafra=<?= urlencode($periodoZafra); ?>&fechaIngreso=<?= urlencode($fechaIngreso); ?>" class="btn btn-secondary">Regresar</a>
                                </div>
                            </form>
                        </div>
                    </div>
