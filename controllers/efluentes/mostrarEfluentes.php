...
if (isset($_GET['periodoZafra']) && !empty($_GET['periodoZafra']) && isset($_GET['fechaIngreso']) && !empty($_GET['fechaIngreso'])) {
    try {
        ...
        $query = "SELECT idEfluentes, hora, enfriamiento, retorno, desechos, observacion, fechaIngreso, periodoZafra
                  FROM efluentes 
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <!-- Page Heading -->
                    <h1 class="h3 mb-2 text-gray-800">Registros de Efluentes</h1>
                    ...
                    <?php if (isset($_GET['periodoZafra']) && !empty($_GET['periodoZafra']) && isset($_GET['fechaIngreso']) && !empty($_GET['fechaIngreso'])): ?>
                        <div class="mb-4">
                            <a href="<?= BASE_PATH ?>forms/registrarEfluentes.php?periodoZafra=<?= urlencode($_GET['periodoZafra']); ?>&fechaIngreso=<?= urlencode($_GET['fechaIngreso']); ?>" class="btn btn-new">
                                <i class="fas fa-plus"></i> Nuevo Registro
                            </a>
                        </div>
                    <?php endif; ?>

                    <!-- Mostrar la tabla solo si se ha seleccionado el periodo de zafra y la fecha y hay registros -->
                    <?php if (isset($_GET['periodoZafra']) && !empty($_GET['periodoZafra']) && isset($_GET['fechaIngreso']) && !empty($_GET['fechaIngreso'])): ?>
                        <?php if ($result): ?>
                            <div class="card shadow mb-4">
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table id="tablaEfluentes" class="table table-bordered" width="100%" cellspacing="0">
                                            ...
                                            <tbody>
                                                ...
                                                $sumaEnfriamiento = $sumaRetorno = $sumaDesechos = 0;
                                                $countEnfriamiento = $countRetorno = $countDesechos = 0;
                                                ...
                                                foreach ($horas as $hora) {
                                                    if (isset($dataHora[$hora])) {
                                                        ...
                                                        $enfriamiento = formatear_valor($row['enfriamiento']);
                                                        $retorno = formatear_valor($row['retorno']);
                                                        $desechos = formatear_valor($row['desechos']);
                                                        $observacion = !empty($row['observacion']) ? htmlspecialchars($row['observacion']) : '-';
                                                        $idEfluentes = $row['idEfluentes'];

                                                        // Acumulación para el promedio
                                                        if ($row['enfriamiento'] != 0) {
                                                            $sumaEnfriamiento += $row['enfriamiento'];
                                                            $countEnfriamiento++;
                                                        }
                                                        if ($row['retorno'] != 0) {
                                                            $sumaRetorno += $row['retorno'];
                                                            $countRetorno++;
                                                        }
                                                        if ($row['desechos'] != 0) {
                                                            $sumaDesechos += $row['desechos'];
                                                            $countDesechos++;
                                                        }
                                                        ...
                                                        echo "<tr>
                                                            ...
                                                            <td>
                                                                <a href='" . BASE_PATH . "controllers/efluentes/editarEfluentes.php?id={$idEfluentes}' class='text-warning me-2 action-icons'>
                                                                    <i class='fas fa-edit'></i>
                                                                </a>
                                                                <a href='" . BASE_PATH . "controllers/efluentes/eliminarEfluentes.php?id={$idEfluentes}&periodoZafra={$periodoZafra}&fechaIngreso={$fechaIngreso}' class='text-danger action-icons' onclick='return confirm(\"¿Estás seguro de eliminar este registro?\")'>
                                                ...
                                                // Fila de promedios
                                                if ($result) {
                                                    $promEnfriamiento = ($countEnfriamiento > 0) ? formatear_valor($sumaEnfriamiento / $countEnfriamiento) : "-";
                                                    $promRetorno = ($countRetorno > 0) ? formatear_valor($sumaRetorno / $countRetorno) : "-";
                                                    $promDesechos = ($countDesechos > 0) ? formatear_valor($sumaDesechos / $countDesechos) : "-";

                                                    echo "<tr class='table-info'>
                                                        <td>Promedio</td>
                                                        <td>{$promEnfriamiento}</td>
                                                        <td>{$promRetorno}</td>
                                                        <td>{$promDesechos}</td>
                                                        <td></td>
                                                        <td></td>
                        ...
                        <?php else: ?>
                            <div class="alert alert-info">No hay registros para el periodo de zafra y fecha seleccionados.</div>
                        <?php endif; ?>
                    <?php endif; ?>
                </div>
            </div>
