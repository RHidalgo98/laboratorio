...
if (isset($_GET['id']) && isset($_GET['periodoZafra']) && isset($_GET['fechaIngreso'])) {
    ...
    try {
        ...
        $stmtSelect = $conexion->prepare("SELECT * FROM jugoprimario WHERE idJugoPrimario = :id");
        $stmtSelect->bindParam(':id', $idJugoPrimario, PDO::PARAM_INT);
        $stmtSelect->execute();
        ...
        if ($registro) {
            ...
            $stmtDelete = $conexion->prepare("DELETE FROM jugoprimario WHERE idJugoPrimario = :id");
            $stmtDelete->bindParam(':id', $idJugoPrimario, PDO::PARAM_INT);

            if ($stmtDelete->execute()) {
                // Registrar en bitácora
                registrarBitacora(
                    $_SESSION['nombre'], // Asegurar que existe en la sesión
                    "Eliminación de registro en Jugo Primario",
                    $idJugoPrimario,
                    "jugoprimario",
                    $detalles
                );
                ...
                $redirectUrl = BASE_PATH . "controllers/jugoPrimario/mostrarJugoPrimario.php?mensaje=Registro+eliminado+correctamente";
...
// Manejo de errores
if (!empty($error)) {
    $redirectUrl = BASE_PATH . "controllers/jugoPrimario/mostrarJugoPrimario.php?error=" . urlencode($error);
    $redirectUrl .= "&periodoZafra=" . urlencode($periodoZafra);
    $redirectUrl .= "&fechaIngreso=" . urlencode($fechaIngreso);
    header("Location: $redirectUrl");
    exit();
