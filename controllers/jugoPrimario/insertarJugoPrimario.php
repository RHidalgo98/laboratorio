...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($periodoZafra) || empty($fechaIngreso) || empty($hora) || empty($brix) || empty($sac)) {
        ...
        header("Location: " . BASE_PATH . "forms/registrarJugoPrimario.php?id=" . urlencode($idJugoPrimario) . "&error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
        exit();
    }
    ...
    if (!empty($error)) {
        header("Location: " . BASE_PATH . "forms/registrarJugoPrimario.php?error=" . urlencode($error)
    ...
    try {
        ...
        if ($count > 0) {
            $error = "Ya existe un registro para esta hora y fecha. Por favor, elija otra hora.";
            header("Location: " . BASE_PATH . "forms/registrarJugoPrimario.php?error=" . urlencode($error)
                . "&periodoZafra=" . urlencode($periodoZafra)
                . "&fechaIngreso=" . urlencode($fechaIngreso)
                . "&hora=" . urlencode($hora)
            ...
            // SQL de inserción
            $sql = "INSERT INTO jugoprimario (hora, observacion, brix, sac, mlGastado, periodoZafra, fechaIngreso)
            ...
            if ($stmt->execute()) {
                ...
                registrarBitacora(
                    $_SESSION['nombre'],
                    "Inserción de registro en Jugo Primario", 
                    $idRegistro,
                    "jugoprimario", 
                    $detallesCambio
                );
                header("Location: " . BASE_PATH . "controllers/jugoPrimario/mostrarJugoPrimario.php?mensaje=Registro+insertado+correctamente&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
                exit();
            } else {
    ...
    header("Location: " . BASE_PATH . "forms/registrarJugoPrimario.php?error=" . urlencode("Solicitud no válida."));
    exit();
}
