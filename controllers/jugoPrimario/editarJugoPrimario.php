...
if (isset($_GET['id'])) {
    $idJugoPrimario = $_GET['id'];

    // Obtener los datos actuales del registro
    try {
        $stmt = $conexion->prepare("SELECT * FROM jugoprimario WHERE idJugoPrimario = :idJugoPrimario");
        $stmt->bindParam(':idJugoPrimario', $idJugoPrimario, PDO::PARAM_INT);
        $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $check_sql = "SELECT COUNT(*) FROM jugoprimario 
                      WHERE hora = :hora 
                      AND periodoZafra = :periodoZafra 
                      AND fechaIngreso = :fechaIngreso 
                      AND idJugoPrimario != :idJugoPrimario";
        ...
        $check_stmt->bindParam(':idJugoPrimario', $idJugoPrimario);
        $check_stmt->execute();
        $count = $check_stmt->fetchColumn();

        if ($count > 0) {
            $error = "Ya existe un registro con la misma hora, periodo de zafra y fecha.";
            header("Location: " . BASE_PATH . "controllers/jugoPrimario/editarJugoPrimario.php?id=" . urlencode($idJugoPrimario) . "&error=" . urlencode($error));
            exit();
        }
    } catch (PDOException $e) {
    ...
    if (empty($periodoZafra) || empty($fechaIngreso) || empty($hora) || empty($brix) || empty($sac)) {
        ...
        header("Location: " . BASE_PATH . "controllers/jugoPrimario/editarJugoPrimario.php?id=" . urlencode($idJugoPrimario) . "&error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
        exit();
    }
    ...
    try {
        $sql = "UPDATE jugoprimario 
                SET hora = :hora, 
                    observacion = :observacion, 
                    brix = :brix, 
                    sac = :sac, 
                    mlGastado = :mlGastado 
                WHERE idJugoPrimario = :idJugoPrimario";
        ...
        $stmt->bindParam(':idJugoPrimario', $idJugoPrimario);
        ...
        if ($stmt->execute()) {
            ...
            if (!empty($detallesCambio)) {
                registrarBitacora(
                    $_SESSION['nombre'],
                    "Actualización de registro en Jugo Primario",
                    $idJugoPrimario,
                    "jugoprimario",
                    implode(", ", $detallesCambio)
                );
            }
            $url = BASE_PATH . "controllers/jugoPrimario/mostrarJugoPrimario.php?mensaje=Registro+actualizado+correctamente";
...
<head>
    ...
    <title>Editar Registro de Jugo Primario</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-2 text-gray-800">Editar Registro de Jugo Primario</h1>
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-body">
                            <form action="<?php echo BASE_PATH; ?>controllers/jugoPrimario/editarJugoPrimario.php?id=<?php echo $idJugoPrimario; ?>" method="POST">
                                <div class="mb-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <a href="<?= BASE_PATH ?>controllers/jugoPrimario/mostrarJugoPrimario.php?periodoZafra=<?= urlencode($periodoZafra); ?>&fechaIngreso=<?= urlencode($fechaIngreso); ?>" class="btn btn-secondary">Regresar</a>
                                </div>
                            </form>
                        </div>
                    </div>
