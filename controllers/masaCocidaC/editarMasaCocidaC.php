...
if (isset($_GET['id'])) {
    ...
    try {
        $stmt = $conexion->prepare("SELECT * FROM masacocidac WHERE idMasaCocidaC = :idMasaCocidaC");
        $stmt->bindParam(':idMasaCocidaC', $idMasaCocidaC, PDO::PARAM_INT);
        $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($error)) {
        try {
            ...
            $check_stmt->bindParam(':idMasaCocidaC', $idMasaCocidaC);
            $check_stmt->execute();
    ...
    if (empty($periodoZafra) || empty($fechaIngreso) || empty($num) || empty($hora) || empty($tacho) || empty($volFt3) || empty($brix) || empty($pol)) {
        ...
        header("Location: " . BASE_PATH . "controllers/masaCocidaC/editarMasaCocidaC.php?id=" . urlencode($idMasaCocidaC) . "&error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
        exit();
    }
    ...
    if (empty($error)) {
        try {
            $sql = "UPDATE masacocidac 
                    ...
                    WHERE idMasaCocidaC = :idMasaCocidaC";
            ...
            $stmt->bindParam(':idMasaCocidaC', $idMasaCocidaC);
            ...
            registrarBitacora(
                $_SESSION['nombre'],
                "Edición de registro en Masa Cocida C",
                $idMasaCocidaC,
                "masacocidac",
                $detallesCambio
            );
            ...
            $redirectUrl = BASE_PATH . "controllers/masaCocidaC/mostrarMasaCocidaC.php?mensaje=Registro+actualizado+correctamente";
        ...
        header("Location: " . BASE_PATH . "controllers/masaCocidaC/editarMasaCocidaC.php?id=" . urlencode($idMasaCocidaC) . "&error=" . urlencode($error));
        exit();
    }
}
?>
...
<head>
    ...
    <title>Editar Masa Cocida C</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-2 text-gray-800">Editar Registro de Masa Cocida C</h1>
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-body">
                            <form action="" method="POST">
                                ...
                                <a href="<?= BASE_PATH ?>controllers/masaCocidaC/mostrarMasaCocidaC.php?periodoZafra=<?= urlencode($periodoZafra); ?>&fechaIngreso=<?= urlencode($fechaIngreso); ?>" class="btn btn-secondary">Regresar</a>
                            </form>
                        </div>
                    </div>
                </div>
