...
if (isset($_GET['id']) && isset($_GET['periodoZafra']) && isset($_GET['fechaIngreso'])) {
    ...
    try {
        ...
        $stmtSelect = $conexion->prepare("SELECT * FROM mielb WHERE idMielB = :id");
        $stmtSelect->bindParam(':id', $idMielB, PDO::PARAM_INT);
        $stmtSelect->execute();
        ...
        if ($registro) {
            ...
            $stmtDelete = $conexion->prepare("DELETE FROM mielb WHERE idMielB = :id");
            $stmtDelete->bindParam(':id', $idMielB, PDO::PARAM_INT);

            if ($stmtDelete->execute()) {
                // Registrar en bitácora
                registrarBitacora(
                    $_SESSION['nombre'], // Asegurar que existe en la sesión
                    "Eliminación de registro en Miel B",
                    $idMielB,
                    "mielb",
                    $detalles
                );
                ...
                $redirectUrl = BASE_PATH . "controllers/mielB/mostrarMielB.php?mensaje=Registro+eliminado+correctamente";
...
// Manejo de errores
if (!empty($error)) {
    $redirectUrl = BASE_PATH . "controllers/mielB/mostrarMielB.php?error=" . urlencode($error);
    $redirectUrl .= "&periodoZafra=" . urlencode($periodoZafra);
    $redirectUrl .= "&fechaIngreso=" . urlencode($fechaIngreso);
    header("Location: $redirectUrl");
    exit();
