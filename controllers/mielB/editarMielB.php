...
if (isset($_GET['id'])) {
    ...
    try {
        $stmt = $conexion->prepare("SELECT * FROM mielb WHERE idMielB = :idMielB");
        $stmt->bindParam(':idMielB', $idMielB, PDO::PARAM_INT);
        $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($error)) {
        try {
            $check_sql = "SELECT COUNT(*) FROM mielb
            ...
            $check_stmt->bindParam(':idMielB', $idMielB);
            $check_stmt->execute();
    ...
    if (empty($periodoZafra) || empty($fechaIngreso) || empty($hora) || empty($num) || empty($brix) || empty($pol)) {
        ...
        header("Location: " . BASE_PATH . "controllers/mielB/editarMielB.php?id=" . urlencode($idMielB) . "&error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
        exit();
    }

    // Validar que Brix y Pol no sean negativos
    if (empty($error)) {
        if ($brix <= 0) {
    ...
    if (empty($error)) {
        try {
            ...
            $stmt->bindParam(':idMielB', $idMielB);
            ...
            $redirectUrl = BASE_PATH . "controllers/mielB/mostrarMielB.php?mensaje=Registro+actualizado+correctamente";
        ...
        header("Location: " . BASE_PATH . "controllers/mielB/editarMielB.php?id=" . urlencode($idMielB) . "&error=" . urlencode($error));
        exit();
    }
}
?>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-2 text-gray-800">Editar Registro de Miel B</h1>
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-body">
                            <form action="" method="POST">
                                ...
                                <a href="<?= BASE_PATH ?>controllers/mielB/mostrarMielB.php?periodoZafra=<?= urlencode($periodoZafra); ?>&fechaIngreso=<?= urlencode($fechaIngreso); ?>" class="btn btn-secondary">Regresar</a>
                            </form>
                        </div>
                    </div>
                </div>
