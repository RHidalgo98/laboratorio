...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $valorInicialQuery = "SELECT valorInicial FROM aguaimbibicion WHERE periodoZafra = :periodoZafra AND fechaIngreso = :fechaIngreso LIMIT 1";
    ...
    try {
        ...
        $checkQuery = "SELECT COUNT(*) FROM aguaimbibicion WHERE hora = :hora AND periodoZafra = :periodoZafra AND fechaIngreso = :fechaIngreso";
        ...
        if ($insertStmt->execute()) {
            ...
            $detallesCambio = "Hora: $hora, Marcador: $marcador, Totalizador: $totalizador, ValorInicial: $valorInicial, Observación: $observacion";

            // Añadir entrada a la bitácora
            registrarBitacora(
                $_SESSION['nombre'],
                "Inserción de registro en Agua Imbibición", 
                $idRegistro,
                "aguaimbibicion", 
                $detallesCambio
            );
            // Redirigir al formulario de mostrar con un mensaje de éxito
            header("Location: " . BASE_PATH . "controllers/aguaimbibicion/mostrarAguaImbibicion.php?periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso) . "&mensaje=Registro+insertado+correctamente");
            exit();
        } else {
