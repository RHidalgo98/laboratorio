...
if (!empty($periodoZafra) && !empty($fechaIngreso)) {
    $query = "SELECT DATE_FORMAT(hora, '%H:%i') AS hora, marcador, totalizador, valorInicial, observacion, idAguaImbibicion
              FROM aguaimbibicion 
...
foreach ($registros as $registro) {
    ...
    if (isset($datosPorHora[$hora])) {
        ...
        $datosPorHora[$hora]['idAguaImbibicion'] = $registro['idAguaImbibicion'];
        $datosPorHora[$hora]['observacion'] = $registro['observacion'];
    }
}

// Inicializar variables de agua imbibicion
...
foreach ($datosPorHora as &$registro) {
    if ($registro['totalizador'] !== null) {
        if ($ultimoTotalizador === null) {
            ...
            $registro['tnHora'] = ($diferencia < 0) ? "" : round($diferencia * $factorConversion, 3);  // <- Cambio clave
        } else {
...
{
    if ($valor === null && !$hayRegistro) {
        return "-";
    }
    return ($valor === null && $hayRegistro) ? number_format(0, $decimales) : number_format(floatval($valor), $decimales);
}
?>
...
<head>
    ...
    <title>Registros de Agua de Imbibición</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-2 text-gray-800">Registros de Agua de Imbibición</h1>
                    ...
                    <?php if (!empty($periodoZafra) && !empty($fechaIngreso)): ?>
                        <div class="mb-4">
                            <a href="<?= BASE_PATH ?>forms/registrarAguaImbibicion.php?periodoZafra=<?= urlencode($periodoZafra); ?>&fechaIngreso=<?= urlencode($fechaIngreso); ?>" class="btn btn-primary">
                                            ...
                                            <tbody>
                                                <?php foreach ($datosPorHora as $registro): ?>
                                                    <tr>
                                                        ...
                                                        <td><?= $registro['idAguaImbibicion'] !== null ? formatear_valor($registro['tnHora'], 0, true) : "-" ?></td>
                                                        <td class="observacion"><?= htmlspecialchars(!empty($registro['observacion']) ? $registro['observacion'] : '-') ?></td>
                                                        <td>
                                                            <?php if ($registro['idAguaImbibicion'] !== null): ?>
                                                                <a href="<?= BASE_PATH ?>controllers/aguaImbibicion/editarAguaImbibicion.php?id=<?= $registro['idAguaImbibicion'] ?>" class="text-warning me-2">
                                                                    <i class="fas fa-edit"></i>
                                                                </a>
                                                                <a href="<?= BASE_PATH ?>controllers/aguaImbibicion/eliminarAguaImbibicion.php?id=<?= $registro['idAguaImbibicion'] ?>&periodoZafra=<?= urlencode($periodoZafra) ?>&fechaIngreso=<?= urlencode($fechaIngreso) ?>"
                                                                    class="text-danger"
                                                ...
                                                <tr class="table-info">
                                                    ...
                                                    <td></td>
                                                    <td></td>
                                                    <td><?= formatear_valor($promedios['tnHora'], 3, true) ?></td>
                                                    <td></td>
                                                    <td></td>
                                                </tr>
                                            </tbody>
