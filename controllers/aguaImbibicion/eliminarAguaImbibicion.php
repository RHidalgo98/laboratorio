...
if (isset($_GET['id']) && isset($_GET['periodoZafra']) && isset($_GET['fechaIngreso'])) {
    ...
    try {
        ...
        $stmtSelect = $conexion->prepare("SELECT * FROM aguaimbibicion WHERE idAguaImbibicion = :id");
        $stmtSelect->bindParam(':id', $idAguaImbibicion, PDO::PARAM_INT);
        $stmtSelect->execute();
        ...
        if ($registro) {
            ...
            $detalles .= "ValorInicial: {$registro['valorInicial']}, ";
            $detalles .= "Observación: {$registro['observacion']}";

            // Eliminar el registro
            $stmtDelete = $conexion->prepare("DELETE FROM aguaimbibicion WHERE idAguaImbibicion = :id");
            $stmtDelete->bindParam(':id', $idAguaImbibicion, PDO::PARAM_INT);

            if ($stmtDelete->execute()) {
                // Registrar en bitácora
                registrarBitacora(
                    $_SESSION['nombre'], // Asegurar que existe en la sesión
                    "Eliminación de registro en Agua Imbibición",
                    $idAguaImbibicion,
                    "aguaimbibicion",
                    $detalles
                );
                ...
                $redirectUrl = BASE_PATH . "controllers/aguaImbibicion/mostrarAguaImbibicion.php?mensaje=Registro+eliminado+correctamente";
...
// Manejo de errores
if (!empty($error)) {
    $redirectUrl = BASE_PATH . "controllers/aguaImbibicion/mostrarAguaImbibicion.php?error=" . urlencode($error);
    $redirectUrl .= "&periodoZafra=" . urlencode($periodoZafra);
    $redirectUrl .= "&fechaIngreso=" . urlencode($fechaIngreso);
    header("Location: $redirectUrl");
    exit();
