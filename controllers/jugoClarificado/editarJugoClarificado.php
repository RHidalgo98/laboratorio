...
if (isset($_GET['id'])) {
    ...
    try {
        $stmt = $conexion->prepare("SELECT * FROM jugoclarificado WHERE idJugoClarificado = :idJugoClarificado");
        $stmt->bindParam(':idJugoClarificado', $idJugoClarificado, PDO::PARAM_INT);
        $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $check_sql = "SELECT COUNT(*) FROM jugoclarificado 
                      WHERE hora = :hora 
                      AND periodoZafra = :periodoZafra 
                      AND fechaIngreso = :fechaIngreso 
                      AND idJugoClarificado != :idJugoClarificado";
        ...
        $check_stmt->bindParam(':idJugoClarificado', $idJugoClarificado);
        $check_stmt->execute();
        $count = $check_stmt->fetchColumn();

        if ($count > 0) {
            $error = "Ya existe un registro con la misma hora, periodo de zafra y fecha.";
            header("Location: " . BASE_PATH . "controllers/jugoClarificado/editarJugoClarificado.php?id=" . urlencode($idJugoClarificado) . "&error=" . urlencode($error));
            exit();
        }
    } catch (PDOException $e) {
    ...
    if (empty($periodoZafra) || empty($fechaIngreso) || empty($hora) || empty($brix) || empty($sac)) {
        ...
        header("Location: " . BASE_PATH . "controllers/jugoClarificado/editarJugoClarificado.php?id=" . urlencode($idJugoClarificado) . "&error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
        exit();
    }
    ...
    try {
        $sql = "UPDATE jugoclarificado 
                SET hora = :hora, 
                    ...
                    sac = :sac, 
                    mlGastado = :mlGastado 
                WHERE idJugoClarificado = :idJugoClarificado";
        ...
        $stmt->bindParam(':idJugoClarificado', $idJugoClarificado);

        if ($stmt->execute()) {
            $detallesCambio = [];

            // Comparar cambios en cada campo y registrar solo si hubo cambios
            ...
            if (!empty($detallesCambio)) {
                registrarBitacora(
                    $_SESSION['nombre'],
                    "Edición de registro en Jugo Clarificado",
                    $idJugoClarificado,
                    "jugoclarificado",
                    implode(", ", $detallesCambio)
                );
            }
            $url = BASE_PATH . "controllers/jugoClarificado/mostrarJugoClarificado.php?mensaje=Registro+actualizado+correctamente";
...
<head>
    ...
    <title>Editar Registro de Jugo Clarificado</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-2 text-gray-800">Editar Registro de Jugo Clarificado</h1>
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-body">
                            <form action="<?php echo BASE_PATH; ?>controllers/jugoClarificado/editarJugoClarificado.php?id=<?php echo $idJugoClarificado; ?>" method="POST">
                                <div class="mb-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <a href="<?= BASE_PATH ?>controllers/jugoClarificado/mostrarJugoClarificado.php?periodoZafra=<?= urlencode($periodoZafra); ?>&fechaIngreso=<?= urlencode($fechaIngreso); ?>" class="btn btn-secondary">Regresar</a>
                                </div>
                            </form>
                        </div>
                    </div>
