...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($periodoZafra) || empty($fechaIngreso) || empty($hora) || empty($brix) || empty($sac)) {
        ...
        header("Location: " . BASE_PATH . "forms/registrarJugoClarificado.php?id=" . urlencode($idJugoClarificado) . "&error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
        exit();
    }
    ...
    if (!empty($error)) {
        header("Location: " . BASE_PATH . "forms/registrarJugoClarificado.php?error=" . urlencode($error) 
    ...
    try {
        ...
        if ($count > 0) {
            ...
            // SQL de inserción
            $sql = "INSERT INTO jugoclarificado (hora, observacion, color, brix, sac, mlGastado, periodoZafra, fechaIngreso)
            ...
            if ($stmt->execute()) {
                ...
                $detallesCambio = "Hora: $hora, Observación: $observacion, Color: $color, Brix: $brix, SAC: $sac, ML Gastado: $mlGastado";

                // Añadir entrada a la bitácora
                registrarBitacora(
                    $_SESSION['nombre'],
                    "Inserción de registro en Jugo Clarificado", 
                    $idRegistro,
                    "jugoclarificado", 
                    $detallesCambio
                );
                header("Location: " . BASE_PATH . "controllers/jugoClarificado/mostrarJugoClarificado.php?mensaje=Registro+insertado+correctamente&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
                exit();
            } else {
    ...
    header("Location: " . BASE_PATH . "forms/registrarJugoClarificado.php?error=" . urlencode("Solicitud no válida."));
    exit();
}
?>
