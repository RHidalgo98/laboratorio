...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($periodoZafra) || empty($fechaIngreso) || empty($hora) || empty($brix) || empty($sac)) {
        ...
        header("Location: " . BASE_PATH . "forms/registrarJugoResidual.php?id=" . urlencode($idJugoResidual) . "&error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
        exit();
    }
    ...
    if (!empty($error)) {
        header("Location: " . BASE_PATH . "forms/registrarJugoResidual.php?error=" . urlencode($error) 
    ...
    try {
        ...
        if ($count > 0) {
            ...
            // SQL de inserción
            $sql = "INSERT INTO jugoresidual (hora, observacion, brix, sac, mlGastado, periodoZafra, fechaIngreso)
            ...
            if ($stmt->execute()) {
                ...
                registrarBitacora(
                    $_SESSION['nombre'],
                    "Inserción de registro en Jugo Residual", 
                    $idRegistro,
                    "jugoresidual", 
                    $detallesCambio
                );
                header("Location: " . BASE_PATH . "controllers/jugoResidual/mostrarJugoResidual.php?mensaje=Registro+insertado+correctamente&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
                exit();
            } else {
    ...
    header("Location: " . BASE_PATH . "forms/registrarJugoResidual.php?error=" . urlencode("Solicitud no válida."));
    exit();
}
?>
