...
if (isset($_GET['id']) && isset($_GET['periodoZafra']) && isset($_GET['fechaIngreso'])) {
    ...
    try {
        ...
        $stmtSelect = $conexion->prepare("SELECT * FROM jugoresidual WHERE idJugoResidual = :id");
        $stmtSelect->bindParam(':id', $idJugoResidual, PDO::PARAM_INT);
        $stmtSelect->execute();
        ...
        if ($registro) {
            ...
            $stmtDelete = $conexion->prepare("DELETE FROM jugoresidual WHERE idJugoResidual = :id");
            $stmtDelete->bindParam(':id', $idJugoResidual, PDO::PARAM_INT);

            if ($stmtDelete->execute()) {
                // Registrar en bitácora
                registrarBitacora(
                    $_SESSION['nombre'], // Asegurar que existe en la sesión
                    "Eliminación de registro en Jugo Residual",
                    $idJugoResidual,
                    "jugoresidual",
                    $detalles
                );
                ...
                $redirectUrl = BASE_PATH . "controllers/jugoResidual/mostrarJugoResidual.php?mensaje=Registro+eliminado+correctamente";
...
// Manejo de errores
if (!empty($error)) {
    $redirectUrl = BASE_PATH . "controllers/jugoResidual/mostrarJugoResidual.php?error=" . urlencode($error);
    $redirectUrl .= "&periodoZafra=" . urlencode($periodoZafra);
    $redirectUrl .= "&fechaIngreso=" . urlencode($fechaIngreso);
    header("Location: $redirectUrl");
    exit();
