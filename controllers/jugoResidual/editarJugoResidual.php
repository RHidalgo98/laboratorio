...
if (isset($_GET['id'])) {
    $idJugoResidual = $_GET['id'];

    // Obtener los datos actuales del registro
    try {
        $stmt = $conexion->prepare("SELECT * FROM jugoresidual WHERE idJugoResidual = :idJugoResidual");
        $stmt->bindParam(':idJugoResidual', $idJugoResidual, PDO::PARAM_INT);
        $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $check_sql = "SELECT COUNT(*) FROM jugoresidual 
                      WHERE hora = :hora 
                      AND periodoZafra = :periodoZafra 
                      AND fechaIngreso = :fechaIngreso 
                      AND idJugoResidual != :idJugoResidual";
        ...
        $check_stmt->bindParam(':idJugoResidual', $idJugoResidual);
        $check_stmt->execute();
        $count = $check_stmt->fetchColumn();

        if ($count > 0) {
            $error = "Ya existe un registro con la misma hora, periodo de zafra y fecha.";
            header("Location: " . BASE_PATH . "controllers/jugoResidual/editarJugoResidual.php?id=" . urlencode($idJugoResidual) . "&error=" . urlencode($error));
            exit();
        }
    } catch (PDOException $e) {
    ...
    if (empty($periodoZafra) || empty($fechaIngreso) || empty($hora) || empty($brix) || empty($sac)) {
        ...
        header("Location: " . BASE_PATH . "controllers/jugoResidual/editarJugoResidual.php?id=" . urlencode($idJugoResidual) . "&error=" . urlencode($error) . "&periodoZafra=" . urlencode($periodoZafra) . "&fechaIngreso=" . urlencode($fechaIngreso));
        exit();
    }

    // Validar que Color, Brix, SAC y ML Gastado no sean negativos
    if ($brix <= 0) {
        $error = "El valor de Brix no puede ser negativo.";
    } elseif ($sac <= 0) {
        $error = "El valor de SAC no puede ser negativo.";
    } elseif ($mlGastado < 0) {
    ...
    try {
        $sql = "UPDATE jugoresidual 
                SET hora = :hora, 
                    observacion = :observacion, 
                    brix = :brix, 
                    sac = :sac, 
                    mlGastado = :mlGastado 
                WHERE idJugoResidual = :idJugoResidual";
        ...
        $stmt->bindParam(':idJugoResidual', $idJugoResidual);
        ...
        if ($stmt->execute()) {
            ...
            registrarBitacora(
                $_SESSION['nombre'],
                "Actualización de registro en Jugo Residual",
                $idJugoResidual,
                "jugoresidual",
                implode(", ", $detallesCambio)
            );
            }
            $url = BASE_PATH . "controllers/jugoResidual/mostrarJugoResidual.php?mensaje=Registro+actualizado+correctamente";
...
<head>
    ...
    <title>Editar Registro de Jugo Residual</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-2 text-gray-800">Editar Registro de Jugo Residual</h1>
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-body">
                            <form action="<?php echo BASE_PATH; ?>controllers/jugoResidual/editarJugoResidual.php?id=<?php echo $idJugoResidual; ?>" method="POST">
                                <div class="mb-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <a href="<?= BASE_PATH ?>controllers/jugoResidual/mostrarJugoResidual.php?periodoZafra=<?= urlencode($periodoZafra); ?>&fechaIngreso=<?= urlencode($fechaIngreso); ?>" class="btn btn-secondary">Regresar</a>
                                </div>
                            </form>
                        </div>
                    </div>
